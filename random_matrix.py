...
def sym_mat(a):
        #np.random.seed(run)
	k = np.random.randn(a/2,a/2)
	k = k+k.T
	c_1 = np.linalg.eigvalsh(k)
	k = (2*n-1)*k/(max(c_1)-min(c_1))
	return(k)

def makeham(a): #Genrates hamiltonian with inputs J->the interaction strengths with the nearest neighbour, h->Field at each given spin, c->dim of hilbert space. 
	H = np.zeros((a,a))
	k = sym_mat(a)
	l = sym_mat(a)
	H[:a/2,:a/2] += k-np.eye(a/2,a/2)/2 
	H[a/2:,a/2:] += l+np.eye(a/2,a/2)/2
	return(H)
...
def runprog(run):
	...
	for kk in range(len(win)):
		...
		if i==j:
			...
			sz_i   = np.dot(H.T,np.dot(s_zj,H))
	        sz_j   = np.dot(H.T,np.dot(s_zk,H))		
#	        sz_k   = np.dot(H.T,np.dot(s_zk,H))

		tu = np.linspace(0.01,0.5,num=ttt)
		ti = np.linspace(0.01,0.5,num=ttt)#10**tu
...
#		bsii = sz_j*sz_j.T
		...
		#	abd= Ein**t
			...
			t = ti[k]	
			Ein = np.exp(1j*Ei*t)
                        Te = np.tensordot(Ein,np.conj(Ein),axes=0)
                        Sz2 = Szpow4(sz_i,sz_j,Te)
			qszpow4_t[k,:] = [t,16*Sz2/dim]	

#			Sz0  = np.sum(bsii*T)
#			bszpow2_t[k,:] = [t,4*Sz0/dim]

#			Sz2 = Szpow4(sz_k,sz_j,T)
...
def readprog(repeat,w): # reading the repeats
	...
	np.savetxt(ev,e,fmt='%10.15f',delimiter=',')
        sze2 = 'q' + ev + 'szpow2'
	sze4 = 'q' + ev + 'szpow4'	
	sz2=qsz2/repeat
	sz4=qsz4/repeat
...
we = np.linspace(np.log10(0.01),np.log10(0.2),num=1)
win= 10**we
